...
/**
 * JdbcConnectionRepository的sql语句有问题，一直报错
 * @author Mireal Chan
 * @Date 2020-02-25
 * @Version 1.0
 */
@ConditionalOnClass(ConnectionRepository.class)
public class MirealJdbcConnectionRepository implements ConnectionRepository {
    ...
    public MirealJdbcConnectionRepository(String userId, JdbcTemplate jdbcTemplate, ConnectionFactoryLocator connectionFactoryLocator, TextEncryptor textEncryptor, String tablePrefix) {
        this.userId = userId;
    ...
    public void addConnection(Connection<?> connection) {
        try {
            ...
            int rank = jdbcTemplate.queryForObject("select coalesce(max(" + RANK + ") + 1, 1) as ranks from " + tablePrefix + "UserConnection where userId = ? and providerId = ?", new Object[]{userId, data.getProviderId()}, Integer.class);
