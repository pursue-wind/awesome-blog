  ...
  <el-card class="main-card">
    ...
    <el-dialog :visible.sync="addOrEdit" width="60%" top="3vh">
      ...
      <el-form label-width="80px" size="medium" :model="article">
        ...
        <el-form-item label="使用封面">
          <el-card class="img-card">
            <el-image v-for="url in urls" :key="url" :previewSrcList="url"
                      @click="selectArticleCover(url)"
...
export default {
  ...
  methods: {
    ...
    getArticleCover(articleTitle, pn = 1) {
      this.axios.get(`/api/photos/api?q=${articleTitle}&pn=${pn}`).then(({data}) => {
        this.urls = data.data;
      });
    },
    openModel() {
      ...
      this.getArticleCover(this.article.articleTitle.trim())
      this.listCategories();
      this.listTags();
      this.addOrEdit = true;
    },
